...
RSpec.describe 'Van Gogh Paintings Extraction' do
  let(:fixture_path) { File.join(__dir__, 'fixtures', 'van-gogh-paintings.html') }
  let(:expected_path) { File.join(__dir__, 'fixtures', 'van-gogh-expected.json') }
  let(:extractor_options) { { headless: true, verbose: false, wait_time: 5 } }

  before(:all) do
    # Load expected results once for all tests
    @expected_results = JSON.parse(File.read(File.join(__dir__, 'fixtures', 'van-gogh-expected.json')))
  end

  describe 'GoogleSearchExtractor' do
    subject { GoogleSearchExtractor.new(fixture_path, extractor_options) }

    it 'successfully initializes with the van-gogh HTML file' do
      expect(subject).to be_an_instance_of(GoogleSearchExtractor)
    end

    it 'extracts the expected number of van-gogh artworks' do
    ...
    it 'extracts specific well-known van-gogh paintings' do
      results = subject.process
      painting_names = results.map { |r| r[:name] }

      # Check for some famous van Gogh paintings
      expect(painting_names).to include('The Starry Night')
      expect(painting_names).to include('Sunflowers')
    ...
    it 'extracts years as extensions for most artworks' do
      ...
      # Most van Gogh paintings should have year information
      expect(results_with_extensions.length).to be > (results.length * 0.8)

      # Check that extensions contain valid years
      results_with_extensions.each do |result|
        result[:extensions].each do |ext|
          if ext.match?(/^\d{4}$/)
            year = ext.to_i
            expect(year).to be_between(1880, 1890) # van Gogh's active period
          end
        end
      end
    end
